...
class CustomerProfileSettingActivity : BaseActivity<ActivityConsumerProfileSettingBinding>(
    ...
    private fun clickConfirm(){
        binding.tvCheck.setOnClickListener {
            //null값이 아니고, 중복 확인한 값일 경우(중복확인 누르고 값 바꾸는것 방지)
            if (nickname != null && nickname == binding.etNickname.text.toString()){
                val postSignupRequest = PostSignupRequest(nickname = nickname!!, profileImg = profileImg)
    ...
    private fun clickDoubleCheck() {
        binding.btnDoubleCheck.setOnClickListener {
            ...
            val tryNick = PostNickRequest(nickname = binding.etNickname.text.toString())  //새로운 닉네임
            println("$tryNick : 닉네임 들어감")
            SignupService(this).tryPostCheckNick(tryNick)
        }
    }
